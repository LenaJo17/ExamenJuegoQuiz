...
class QuizGame:
    ...
    def nuevo_juego(self):
        ...
        # Registrar usuario en la lista de usuarios actuales (temporal, sin guardar en BD)
        self.usuarios_jugando.append({"nombre": nombre, "nivel": self.nivel, "puntaje": self.puntaje})
    ...
    def mostrar_pregunta(self):
        ...
        # Seleccionar una pregunta aleatoria que no haya sido contestada antes
        ...
        tk.Label(self.root, text=f"Pregunta {self.nivel}", font=("Arial", 18)).pack(pady=10)
    ...
    def verificar_respuesta(self, seleccionada, correcta):
        if seleccionada == correcta:
            messagebox.showinfo("Correcto", "¡Respuesta correcta!")
            self.puntaje += 10
            self.nivel += 1
        else:
            messagebox.showerror("Incorrecto", f"Respuesta incorrecta. La respuesta correcta es: {correcta}")
            self.vidas -= 1

        # Actualizar información del usuario en la lista temporal
    ...
    def mostrar_game_over(self):
        ...
        tk.Label(self.root, text="¡Juego Terminado!", font=("Arial", 18)).pack(pady=10)
        tk.Label(self.root, text=f"Tu puntaje final es: {self.puntaje}", font=("Arial", 14)).pack(pady=10)
        tk.Label(self.root, text=f"Nivel alcanzado: {self.nivel}", font=("Arial", 14)).pack(pady=10)

        # Mostrar opción para reiniciar o salir
        tk.Button(self.root, text="Volver al Inicio", command=self.mostrar_inicio).pack(pady=10)
